...
// .wrangler/tmp/bundle-nEWchP/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-HBQ10s/functionsWorker-0.42698782009783764.mjs
...
var onRequestGet = /* @__PURE__ */ __name2(async (ctx) => {
  ...
  const deny = /* @__PURE__ */ __name2((msg, status) => new Response(msg, { status }), "deny");
...
function sniff(b) {
  ...
  if (b[0] === 255 && (b[1] & 224) === 224)
    return { kind: "audio", ext: "mp3" };
  if (b[0] === 102 && b[1] === 76 && b[2] === 97 && b[3] === 67)
    return { kind: "audio", ext: "flac" };
  return null;
}
__name(sniff, "sniff");
__name2(sniff, "sniff");
function stripJpegExif(b) {
  if (!(b[0] === 255 && b[1] === 216))
    return b;
  const out = [255, 216];
  let i = 2;
  while (i < b.length - 1) {
    ...
    const isMeta = marker === 225 || marker === 226 || marker === 237;
    if (!isMeta)
      out.push(...b.subarray(i, i + 2 + len));
    i += 2 + len;
  }
  return new Uint8Array(out);
}
__name(stripJpegExif, "stripJpegExif");
__name2(stripJpegExif, "stripJpegExif");
var onRequestPost = /* @__PURE__ */ __name2(async (ctx) => {
  const json = /* @__PURE__ */ __name2((body, status = 200) => new Response(JSON.stringify(body), {
    status,
...
// .wrangler/tmp/bundle-nEWchP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nEWchP/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.42698782009783764.js.map
